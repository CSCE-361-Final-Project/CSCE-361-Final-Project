  ...
  useEffect(() => {
    fetch('http://localhost:5000/product/categories')
      ...
      .then(catData => {
        ...
        return fetch('http://localhost:5000/product/sortByCategory')
          .then(res => res.json())
          .then(prodData => {
            const formatted = prodData.slice(0, 6).map(p => ({
              id: p.productID,
              productID: p.productID,
              name: p.name,
              manufacturer: p.details?.manufactuer || "N/A",
              category: catData.find(c => c.categoryID === p.details?.categoryID)?.name || "Featured",
